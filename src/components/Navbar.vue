<template>
	<section>
		<section class="sidebar z-30 position-fixed vh-100 d-flex flex-column top-0 bottom-0 bg-white" v-bind:class="{ 'is-open': isOpen }">
			...
			<div class="sidebar-scroll flex-grow-1 overflow-y-auto">
				<div class="sidebar-menu">
					<ul class="list-unstyled">
						...
						<router-link :to="{ name: 'dashboard' }" @click="markcompleted(todo)" ref="menuitem" tag="li">
							<a><unicon class="pr-3 font-light align-middle" name="keyboard-alt" fill="#000" />Dashboard</a>
						</router-link> -->
						<router-link :to="{ name: 'about' }" tag="li">
							<a><unicon class="pr-3 font-light align-middle" name="books" fill="#000" />Content</a>
						</router-link>
						<!-- <li class="">
			...
			<b-row class="no-gutters box-shadow-top bottom-0 d-flex align-items-center justify-content-center text-decoration-none font-weight-bold bg-offwhite sidebar-footer">
				...
				<b-col cols="8" class="pl-3">
					<div class="username overflow-hidden text-secondary" >
						{{currentUser}}
					</div>
					<small class="w-full overflow-hidden font-weight-bold text-secondary">
		...
		<section class="sidebar-overlay z-50 bg-dark position-fixed top-0 bottom-0 right-0" 
						@click='isOpen = !isOpen' v-bind:class="{'visible': isOpen}"></section>
...
export default {
  ...
  watch: {
	...
	updated() {
		window.console.log(this.isOpen)
	},
	methods: {
    logout: function() {
      firebase
